...
class IngestParams(BaseModel):
    normalize_time: bool = True   # scala TIME → [0, 1]
...
def ingest(params: IngestParams):
    try:
        ...
        for fname, cols in zip(EXT_TEMP_FILES, col_groups):
            df = pd.read_csv(os.path.join(DATA_DIR, fname), sep=";", decimal=",", header=0)
        ...
        df_text = pd.concat(
            [dfs_ext[0]] + [d.iloc[:, 1:] for d in dfs_ext[1:]], axis=1
        )
        df_text["TIME"] = pd.to_datetime(df_text["TIME"], format="%d/%m/%Y %H.%M.%S").astype("int64") // 10**9

        # Temperatura interna TI.csv
        df_tint = pd.read_csv(os.path.join(DATA_DIR, INT_TEMP_FILE), sep=";", decimal=",", header=0)
        df_tint = df_tint.dropna(axis=1, how="all")
        date_col  = df_tint.iloc[:, 0].str.strip()
        time_col  = df_tint.iloc[:, 1].str.strip()
        new_dt    = pd.to_datetime(date_col + " " + time_col, format="%d/%m/%Y %H:%M:%S.%f").astype("int64") // 10**9
        data_cols = df_tint.iloc[:, 2:]
        for c in data_cols.columns:
            data_cols[c] = pd.to_numeric(data_cols[c], errors="coerce")
        df_tint = pd.concat([pd.DataFrame({"TIME": new_dt}), data_cols.reset_index(drop=True)], axis=1)
        df_tint.columns = ["TIME"] + [f"T{i}" for i in range(13, 29)]

        # Displacements.csv
        df_dis = pd.read_csv(os.path.join(DATA_DIR, DISP_FILE), sep=";", header=None)
        ...
        df_dis = pd.DataFrame({"TIME": time_col_dis, "D1": d1, "D2": d2, "D3": d3, "D4": d4, "D5": d5})
        df_dis["TIME"] = pd.to_datetime(df_dis["TIME"], format="%d/%m/%Y %H:%M:%S").astype("int64") // 10**9
